...
void UI::printAll(const MyList<Node>& movies) {
	cout << "----------------------------------------\n\n";
	...
	if (movies.size() == 0)
		...
		Node* node = movies.getHead();
		int i = 0;
		while (node) {
			cout << node->getData().get_title() << " " << node->getData().get_genre() << " "
				 << node->getData().get_year() << " " << node->getData().get_actor() << '\n';
			node = node->next;
		}
		cout << "----------------------------------------\n\n";
	}
...
void UI::filter_UI(const MyList<Node>& movies) {
	int criteria;
	cout << "Filtrati dupa: \n1. Titlu\n2. Anul aparitiei\n";
	cin >> criteria;
	if (criteria != 1 && criteria != 2)
		...
		cin >> substr;
		MyList<Node> new_movies = service.filterMovies(movies, substr, criteria);
		printAll(new_movies);
	}
}

void UI::sort_UI(const MyList<Node>& movies) {
	cout << "Sortati dupa:\n1. Titlu\n2. Actor principal\n3. Anul aparitiei\n4. Gen\n";
	int criteria;
	cin >> criteria;
	...
	MyList<Node> new_movies = service.sortMovies(movies, criteria);
	printAll(new_movies);
}
